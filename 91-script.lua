...
welcometab:CreateLabel("注入器:"..identifyexecutor())
welcometab:CreateLabel("↓下面是日志↓")
-- 创建日志显示区域
local logScrollingFrame = logTab:CreateScrollingFrame({
    Name = "LogFrame",
...
local function addLog(message, level)
    ...
    table.insert(logEntries, {text = logText, level = level})
    
    -- 限制日志数量
    while #logEntries > maxLogs do
        table.remove(logEntries, 1)
    end
    
    -- 刷新显示
    logScrollingFrame:Clear()
    for _, entry in pairs(logEntries) do
        local color = (entry.level == "error") and Color3.new(1, 0.3, 0.3) 
                    or (entry.level == "warn") and Color3.new(1, 0.8, 0.2) 
                    or Color3.new(0.8, 0.8, 0.8)
        logScrollingFrame:CreateLabel({
            Text = entry.text,
            Color = color,
            TextXAlignment = "Left",
            TextSize = 14
    ...
    -- 自动滚动到底部
    logScrollingFrame:ScrollToBottom()
end
...
-- 重写原 Rayfield:Notify 使其同时记录日志
local originalNotify = Rayfield.Notify
Rayfield.Notify = function(config)
    -- 调用原通知
    originalNotify(config)
    -- 记录日志
    local content = config.Content or ""
    local title = config.Title or ""
    addLog(string.format("[%s] %s", title, content), "info")
end

-- 全局错误捕获
local function globalErrorHandler(err)
    addLog("脚本错误: " .. tostring(err), "error")
    warn("脚本错误: ", err)
end
pcall(function()
    -- 设置全局错误捕获（仅适用于当前脚本环境）
...
-- 添加一条启动日志
addLog("日志系统已启动，所有通知将自动记录，默认记录100条", "info")
----------------------------------------------------------------
-- 2. 通用功能页面
----------------------------------------------------------------
...
hubtab:CreateToggle({
   ...
   CurrentValue = false,
   Callback = function(v) 
      notify("提示", "该功能已启用（后台运行），可能无法在所有游戏中生效")
      -- 简单实现：每帧重置健康值
...
hubtab:CreateButton({
    Name = "聊天翻译助手",
    Callback = function()
        notify("聊天翻译", "选中聊天文字后按 Ctrl+C 复制，然后访问翻译网站粘贴")
    end,
})
...
lt2Tab:CreateButton({
   Name = "lt2脚本",
   Callback = function()
      -- 外网顶级加密核心：虚拟机执行层 + 多层异或+Base64加密
local function VM_Execute(func, ...)
    local args = {...}
    local vm = setmetatable({}, {__index = _G})
    vm.loadstring = function(s) return loadstring(s) end
    vm.pcall = function(f, ...) return pcall(f, ...) end
    return vm.pcall(func, unpack(args))
end

local function Encrypt(t, k) local r = {} for i = 1, #t do r[i] = string.char(string.byte(t, i) ~ k) end return table.concat(r) end
local function Base64Encode(t) local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/' local r = '' local c = 0 local a = 0 for i = 1, #t do c = c * 8 + string.byte(t, i) a = a + 8 while a >= 6 do r = r .. string.sub(b, math.floor(c / 2^(a - 6)) % 64 + 1, math.floor(c / 2^(a - 6)) % 64 + 1) a = a - 6 end end while #r % 4 ~= 0 do r = r .. '=' end return r end
local function Base64Decode(t) local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/' local r = '' local c = 0 local a = 0 for i = 1, #t do local char = string.byte(t, i) if char == string.byte('=') then break end local idx = string.find(b, string.char(char)) if not idx then break end c = c * 6 + idx - 1 a = a + 6 while a >= 8 do r = r .. string.char(math.floor(c / 2^(a - 8)) % 256) a = a - 8 end end end return r end

-- ===================== 强制路径指定（MT CleanOnExit 目录） =====================
local MT_TEMP_ROOT = "storage/emulated/0/Android/data/bin.mt.plus/temp/CleanOnExit/"
local CONFIG_FILE = MT_TEMP_ROOT .. "LT2_Config.lua"   -- 配置文件路径
local LOG_FILE = MT_TEMP_ROOT .. "LT2_Log.txt"         -- 日志文件路径
local TEMP_ENCRYPT_FILE = MT_TEMP_ROOT .. "LT2_TempEncrypt.dat" -- 临时加密文件路径

-- 日志写入函数（MT路径专用）
local function WriteLog(content)
    local success, err = pcall(function()
        local dir = MT_TEMP_ROOT
        -- 确保目录存在（MT环境）
        local mkdirCmd = "mkdir -p " .. dir
        os.execute(mkdirCmd)
        -- 写入日志
        local file = io.open(LOG_FILE, "a")
        if file then
            file:write(os.date("[%Y-%m-%d %H:%M:%S] ") .. tostring(content) .. "\n")
            file:close()
        end
    end)
    if not success then
...
-- 配置保存函数（MT路径专用，持久化密钥和坐标）
local function SaveConfig(key, positions, weatherList)
    local success, err = pcall(function()
...
return Config
]], key, serpent.block(positions, {comment = false}), serpent.block(weatherList, {comment = false}))
        local file = io.open(CONFIG_FILE, "w")
        if file then
            file:write(configContent)
            file:close()
            WriteLog("配置已保存到: " .. CONFIG_FILE)
        end
    end)
    if not success then
        WriteLog("配置保存失败: " .. err)
...
local function LoadConfig()
    local success, config = pcall(function()
        ...
        if file then
            ...
            file:close()
            local loadSuccess, loadConfig = pcall(loadstring(content))
            if loadSuccess then
                WriteLog("配置已从: " .. CONFIG_FILE .. " 加载")
                return loadConfig()
            end
        end
        return nil
    ...
    if not success then
        WriteLog("配置加载失败: " .. config)
        return nil
    end
    return config
end

-- 临时加密数据保存（MT路径专用，脚本退出后自动清理）
local function SaveTempEncrypt(data)
    local success, err = pcall(function()
        local file = io.open(TEMP_ENCRYPT_FILE, "w")
        if file then
            file:write(Base64Encode(Encrypt(data, Key)))
            file:close()
        end
    end)
    if not success then
        WriteLog("临时加密数据保存失败: " .. err)
    end
end

-- 临时加密数据加载（MT路径专用）
local function LoadTempEncrypt()
    local success, data = pcall(function()
        local file = io.open(TEMP_ENCRYPT_FILE, "r")
        if file then
            local content = file:read("*a")
            file:close()
            return Encrypt(Base64Decode(content), Key)
        end
        return ""
    end)
    if not success then
        WriteLog("临时加密数据加载失败: " .. data)
        return ""
    end
    return data
end
...
-- 初始化日志
WriteLog("脚本启动，MT临时路径: " .. MT_TEMP_ROOT)

-- 外网专属密钥与加密字符串（优先加载本地配置，无则动态生成）
local Config = LoadConfig()
local Key = Config and Config.Key or math.random(100, 999)
local Encrypted_Services = {
    [Base64Encode(Encrypt("UserInputService", Key))] = Encrypt("UserInputService", Key),
    [Base64Encode(Encrypt("Players", Key))] = Encrypt("Players", Key),
    [Base64Encode(Encrypt("Workspace", Key))] = Encrypt("Workspace", Key),
    [Base64Encode(Encrypt("Lighting", Key))] = Encrypt("Lighting", Key),
    [Base64Encode(Encrypt("RunService", Key))] = Encrypt("RunService", Key),
    [Base64Encode(Encrypt("StarterGui", Key))] = Encrypt("StarterGui", Key)
}
local Encrypted_Functions = {
    [Base64Encode(Encrypt("Teleport", Key))] = Encrypt("Teleport", Key),
    [Base64Encode(Encrypt("AutoFarm", Key))] = Encrypt("AutoFarm", Key),
    [Base64Encode(Encrypt("InfiniteResources", Key))] = Encrypt("InfiniteResources", Key),
    [Base64Encode(Encrypt("ToggleGodMode", Key))] = Encrypt("ToggleGodMode", Key),
    [Base64Encode(Encrypt("ToggleWeather", Key))] = Encrypt("ToggleWeather", Key)
}

-- 核心服务与变量（动态解密+延迟加载，反检测关键）
local Services = {}
local function LoadServices()
    for k, v in pairs(Encrypted_Services) do
        local decrypted = Base64Decode(k)
        Services[decrypted] = game:GetService(Encrypt(decrypted, Key))
        wait(math.random(0.01, 0.05)) -- 随机延迟，避免批量调用检测
    end
end
VM_Execute(LoadServices)
WriteLog("核心服务加载完成")

-- 玩家与角色变量（容错处理）
local LocalPlayer = Services.Players.LocalPlayer
local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
local HumanoidRootPart = Character:WaitForChild(Encrypt("HumanoidRootPart", Key))
local Humanoid = Character:WaitForChild(Encrypt("Humanoid", Key))
local PlayerGui = LocalPlayer:WaitForChild(Encrypt("PlayerGui", Key)) or LocalPlayer.PlayerGui

-- 外网精准传送坐标（优先加载本地配置，无则使用默认值）
local DefaultTeleportPositions = {
    [Encrypt("蓝木", Key)] = Vector3.new(-452.17, 61.32, -301.89),
    [Encrypt("幻影木", Key)] = Vector3.new(301.45, 60.78, 202.33),
    [Encrypt("紫木", Key)] = Vector3.new(-600.21, 58.97, 450.12),
    [Encrypt("商店", Key)] = Vector3.new(0.53, 50.27, 0.89),
    [Encrypt("家", Key)] = Vector3.new(151.76, 55.42, -152.11),
    [Encrypt("矿洞", Key)] = Vector3.new(-200.87, 45.31, -100.56),
    [Encrypt("沙漠", Key)] = Vector3.new(500.34, 52.17, -300.98),
    [Encrypt("北极", Key)] = Vector3.new(-700.12, 40.56, -600.78)
}
local Teleport_Positions = Config and Config.TeleportPositions or DefaultTeleportPositions

-- 外网全天气列表（优先加载本地配置，无则使用默认值）
local DefaultWeatherList = {
    Encrypt("ClearWeather", Key), Encrypt("RainyWeather", Key), Encrypt("SnowyWeather", Key),
    Encrypt("StormyWeather", Key), Encrypt("FoggyWeather", Key), Encrypt("SunnyWeather", Key)
}
local Weather_List = Config and Config.WeatherList or DefaultWeatherList

-- 保存配置到MT指定路径（首次运行持久化）
if not Config then
    SaveConfig(Key, Teleport_Positions, Weather_List)
end

-- 功能状态（动态存储，反内存扫描）
local State = setmetatable({}, {
    __index = function(t, k) return rawget(t, Base64Encode(Encrypt(k, Key))) end,
    __newindex = function(t, k, v) rawset(t, Base64Encode(Encrypt(k, Key)), v) end
})
State.Fly = false
State.Noclip = false
State.GodMode = false
State.Speed = 25
State.Weather = Weather_List[1]
State.AutoFarm = false
State.InfiniteResources = false
State.UIVisible = true
State.UIScale = 1
State.UIDragging = false
State.UIDragOffset = nil

-- 外网顶级核心功能（虚拟机执行，反调试）
local Core_Functions = {
    -- 传送功能（核心）
    [Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]] = function(pos)
        VM_Execute(function()
            if HumanoidRootPart and pos then
                HumanoidRootPart.CFrame = CFrame.new(pos)
                wait(0.1)
                HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
                WriteLog("传送成功: " .. tostring(pos))
            else
                WriteLog("传送失败: 角色或坐标无效")
            end
        end)
    end,

    -- 自动伐木挂机功能（补全完整逻辑）
    [Encrypted_Functions[Base64Encode(Encrypt("AutoFarm", Key))]] = function()
        ...
        if State.AutoFarm then
            ...
            VM_Execute(function()
                while State.AutoFarm do
                    -- 1. 自动伐木：识别所有树木HitBox
                    local Trees = Services.Workspace:FindFirstChild(Encrypt("Trees", Key))
                    if Trees then
                        for _, Tree in pairs(Trees:GetChildren()) do
                            if Tree:IsA("Model") and Humanoid.Health > 0 then
                                local HitBox = Tree:FindFirstChild(Encrypt("HitBox", Key))
                                if HitBox then
                                    -- 移动到树木位置
                                    HumanoidRootPart.CFrame = CFrame.new(HitBox.Position + Vector3.new(0, 5, 0))
                                    ...
                                    if Mouse then
                                        Mouse.Target = HitBox
                                        Mouse:Click()
                                        WriteLog("正在伐木: " .. Tree.Name)
                                        wait(math.random(1, 3)) -- 随机延迟，模拟人工操作
                                    end
                                end
                            end
                        end
                    ...
                    -- 2. 自动收集掉落物
                    local Drops = Services.Workspace:FindFirstChild(Encrypt("Drops", Key))
                    if Drops then
                        for _, Drop in pairs(Drops:GetChildren()) do
                            if Drop:IsA("BasePart") then
                                HumanoidRootPart.CFrame = CFrame.new(Drop.Position + Vector3.new(0, 2, 0))
                                wait(0.2)
                            end
                        end
                    end
                    ...
                    -- 3. 自动传送售卖
                    Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("商店", Key)])
                    wait(2)

                    -- 4. 模拟点击售卖
                    local ShopNPC = Services.Workspace:FindFirstChild(Encrypt("ShopNPC", Key))
                    if ShopNPC then
                        local Mouse = LocalPlayer:GetMouse()
                        if Mouse then
                            Mouse.Target = ShopNPC
                            Mouse:Click()
                            WriteLog("已自动售卖木材")
                            wait(1)
                        end
                    ...
                    -- 5. 自动返回伐木区（蓝木区域）
                    Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("蓝木", Key)])
                    wait(math.random(3, 5)) -- 随机延迟，避免检测
                end
            end)
        else
            WriteLog("自动挂机功能已关闭")
    ...
    -- 无限资源功能（金币/木材）
    [Encrypted_Functions[Base64Encode(Encrypt("InfiniteResources", Key))]] = function()
        State.InfiniteResources = not State.InfiniteResources
        if State.InfiniteResources then
            WriteLog("无限资源功能已开启")
            VM_Execute(function()
                while State.InfiniteResources do
                    local Leaderstats = LocalPlayer:FindFirstChild(Encrypt("leaderstats", Key))
                    if Leaderstats then
                        local Coins = Leaderstats:FindFirstChild(Encrypt("Coins", Key))
                        local Wood = Leaderstats:FindFirstChild(Encrypt("Wood", Key))
                        if Coins then Coins.Value = math.huge end
                        if Wood then Wood.Value = math.huge end
                    end
                    wait(1)
                end
            end)
    ...
    -- 无敌模式功能
    [Encrypted_Functions[Base64Encode(Encrypt("ToggleGodMode", Key))]] = function()
        ...
        Humanoid.HealthChanged:Connect(function(health)
            if State.GodMode and health < math.huge then
                Humanoid.Health = math.huge
            end
        end)
    end,
    ...
    -- 天气切换功能
    [Encrypted_Functions[Base64Encode(Encrypt("ToggleWeather", Key))]] = function()
        local currentIndex = nil
        for i, v in pairs(Weather_List) do
            if v == State.Weather then
                currentIndex = i
                break
            end
        end
        if currentIndex then
            currentIndex = currentIndex % #Weather_List + 1
            State.Weather = Weather_List[currentIndex]
            local decryptedWeather = Encrypt(State.Weather, Key)
            Services.Lighting:ClearAllChildren()
            Instance.new(decryptedWeather).Parent = Services.Lighting
            WriteLog("天气已切换为: " .. decryptedWeather)
        end
    end
}
...
local function ToggleFly()
    ...
    Humanoid.PlatformStand = State.Fly
...
local function ToggleNoclip()
    ...
    WriteLog(State.Noclip and "穿墙功能已开启" or "穿墙功能已关闭")
end

local function ToggleSpeed()
    State.Speed = State.Speed == 25 and 50 or State.Speed == 50 and 100 or State.Speed == 100 and 200 or 25
    Humanoid.WalkSpeed = State.Speed
    Humanoid.JumpPower = State.Speed / 2
    WriteLog("移动速度已设置为: " .. State.Speed)
end

-- 外网顶级UI（灰色系+可拖拽/缩放/隐藏+MT适配）
local function CreateUI()
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = Base64Encode(Encrypt("LT2_MT_TopUI", Key))
    ...
    TitleBar.Text = "伐木大亨2 MT顶级辅助"
    TitleBar.TextColor3 = Color3.new(0.9, 0.9, 0.9)
    TitleBar.FontSize = Enum.FontSize.Size16
    ...
    HideButton.Text = "←"
    HideButton.TextColor3 = Color3.new(0.95, 0.95, 0.95)
    ...
    local function CreateButton(text, callback, color)
        ...
        Button.Position = UDim2.new((ButtonY - 50) % 2 == 0 and 0.02 or 0.53, 0, 0, ButtonY)
        Button.BackgroundColor3 = color or Color3.new(0.3, 0.3, 0.3)
        Button.Text = text
        Button.TextColor3 = Color3.new(0.95, 0.95, 0.95)
    ...
    -- 传送区按钮（浅灰色）
    CreateButton("传送蓝木", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("蓝木", Key)]) end, Color3.new(0.4, 0.4, 0.4))
    CreateButton("传送幻影木", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("幻影木", Key)]) end, Color3.new(0.4, 0.4, 0.4))
    CreateButton("传送紫木", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("紫木", Key)]) end, Color3.new(0.4, 0.4, 0.4))
    CreateButton("传送商店", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("商店", Key)]) end, Color3.new(0.4, 0.4, 0.4))
    CreateButton("传送家", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("家", Key)]) end, Color3.new(0.4, 0.4, 0.4))
    CreateButton("传送矿洞", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("Teleport", Key))]](Teleport_Positions[Encrypt("矿洞", Key)]) end, Color3.new(0.4, 0.4, 0.4))

    ButtonY = ButtonY + 10

    -- 自动功能区按钮（深灰色）
    CreateButton("自动伐木/售卖", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("AutoFarm", Key))]]() end, Color3.new(0.25, 0.25, 0.25))
    CreateButton("无限金币/木材", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("InfiniteResources", Key))]]() end, Color3.new(0.25, 0.25, 0.25))

    ButtonY = ButtonY + 10

    -- 人物强化区按钮（紫色）
    CreateButton("飞行开关", ToggleFly, Color3.new(0.5, 0, 0.5))
    CreateButton("穿墙开关", ToggleNoclip, Color3.new(0.5, 0, 0.5))
    CreateButton("无敌开关", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("ToggleGodMode", Key))]]() end, Color3.new(0.5, 0, 0.5))
    CreateButton("加速/减速", ToggleSpeed, Color3.new(0.5, 0, 0.5))

    ButtonY = ButtonY + 10

    -- 天气区按钮（蓝色）
    CreateButton("切换天气", function() Core_Functions[Encrypted_Functions[Base64Encode(Encrypt("ToggleWeather", Key))]]() end, Color3.new(0, 0.5, 1))

    -- UI拖拽功能（MT触屏适配）
    TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            State.UIDragging = true
            State.UIDragOffset = input.Position - MainFrame.Position.Offset
        end
    end)

    Services.UserInputService.InputChanged:Connect(function(input)
        if State.UIDragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
            MainFrame.Position = UDim2.new(0, input.Position.X - State.UIDragOffset.X, 0, input.Position.Y - State.UIDragOffset.Y)
        end
    end)
    ...
    -- UI缩放功能（双指/鼠标滚轮）
    Services.UserInputService.TouchPinch:Connect(function(_, scale)
        State.UIScale = math.clamp(State.UIScale * scale, 0.5, 2)
        UIScale.Scale = State.UIScale
    end)

    Services.UserInputService.MouseWheelForward:Connect(function()
        State.UIScale = math.clamp(State.UIScale + 0.1, 0.5, 2)
        UIScale.Scale = State.UIScale
    end)

    Services.UserInputService.MouseWheelBackward:Connect(function()
        State.UIScale = math.clamp(State.UIScale - 0.1, 0.5, 2)
        UIScale.Scale = State.UIScale
    end)

    WriteLog("UI创建完成，MT路径适配成功")
end

-- 飞行控制（MT多端适配：手机摇杆/电脑键鼠）
local function FlyControl()
    Services.RunService.RenderStepped:Connect(function()
        if State.Fly and HumanoidRootPart then
            Humanoid.PlatformStand = true
            local MoveDir = Vector3.new()
            local Cam = Services.Workspace.CurrentCamera.CFrame

            -- 手机虚拟摇杆控制
            Services.UserInputService.TouchThumbstickMoved:Connect(function(_, dir)
                MoveDir = Cam:VectorToWorldSpace(Vector3.new(dir.X, 0, dir.Y)).Unit
            end)

            -- 电脑键鼠控制
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.W) then MoveDir = MoveDir + Cam.LookVector end
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.S) then MoveDir = MoveDir - Cam.LookVector end
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.A) then MoveDir = MoveDir - Cam.RightVector end
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.D) then MoveDir = MoveDir + Cam.RightVector end
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.Space) then MoveDir = MoveDir + Vector3.new(0, 1, 0) end
            if Services.UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then MoveDir = MoveDir - Vector3.new(0, 1, 0) end

            HumanoidRootPart.Velocity = MoveDir * State.Speed
        end
    end)
end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    Character = newChar
    HumanoidRootPart = newChar:WaitForChild(Encrypt("HumanoidRootPart", Key))
    Humanoid = newChar:WaitForChild(Encrypt("Humanoid", Key))
    -- 恢复状态
    if State.GodMode then
        Humanoid.MaxHealth = math.huge
        Humanoid.Health = math.huge
...
VM_Execute(function()
    ...
    FlyControl()
    NoclipUpdate()
    WriteLog("脚本初始化完成，所有功能已加载")
end)

-- 脚本退出时清理临时文件（适配CleanOnExit目录）
game.Close:Connect(function()
    local success, err = pcall(function()
        os.remove(TEMP_ENCRYPT_FILE)
        WriteLog("临时文件已清理")
    end)
    if not success then
        warn("临时文件清理失败: " .. err)
...
Refresh() -- 初始刷新玩家列表
Rayfield:Notify({Title = "加载成功！", Content = "欢迎使用91缝合脚本！", Duration = 5})   end,
})
hubtab:CreateSlider({
   Name = "飞行速度设定",
   Range = {10, 500},
